...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/12/23 15:40:57 by cchieko           #+#    #+#             */
/*   Updated: 2019/12/23 15:40:59 by cchieko          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	single_stream_recursion(t_list *flist, char *flags)
{
	while (flist)
	{
		if (((t_file_stats*)(flist->content))->file_type == 'd')
		{
			if (!flags || flags[1] != 'a' ||
				(!ft_strequ(((t_file_stats*)(flist->content))->name, ".") &&
				!ft_strequ(((t_file_stats*)(flist->content))->name, "..")))
				recursive_lookthrough(((t_file_stats*)(flist->content)), flags);
			flist->content = NULL;
		}
		flist = flist->next;
	}
